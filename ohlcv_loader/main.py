...
try:
  ...
  if (exchange_name in ccxt.exchanges):
    ...
    ohlcvs_num = mongo_db["ohlcv"][exchange_name][symbol][period].count_documents({})
    ...
    from_timestamp = exchange.parse8601(from_datetime) \
      if ohlcvs_num == 0 else \
        mongo_db["ohlcv"][exchange_name][symbol][period].find_one(sort=[("timestamp", pymongo.DESCENDING)])['timestamp'] + 1
    ...
    while prev_from_timestamp != from_timestamp:
      try:    
        ...
        if len(tohlcv_db_list):
          mongo_db["ohlcv"][exchange_name][symbol][period].insert_many(tohlcv_db_list)
        # loop variables
